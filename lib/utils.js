 ...
 utils.writeTestScript = function writeTestScript (app) {
   ...
   for (let test in app.tests) {
     ...
     if (app.loadtest_path) {
       cmds.push(`/usr/bin/time -v ${app.tests[test]} 2>> ${config.default_dir}/${config.default_output} & sleep 2`);
       ...
       cmds.push(`/usr/bin/time -v ${app.tests[test]} 2>> ${config.default_dir}/${config.default_output}`);
     }
...
utils.loadOptsToString = function loadOptsToString (options) {
  ...
  for (let opt in options) {
    ...
    } else if (typeof options[opt] === 'boolean') {
      if (options[opt] === true) o.push(`${config.loadtest.bindings[opt]}`);
    } else if (opt !== 'target') {
