...
utils.validLoadOpts = function validLoadOpts (options = {}) {
  const dataBearing = ['postFile', 'postBody', 'putFile', 'patchFile', 'patchBody', 'data'];

  const validData = !(utils.any(Object.keys(options), dataBearing) && !options.type);
  ...
  return hasRequired && validHeaders && validCookies && validData;
}
...
utils.any = function any (src_list = [], comparison_list = []) {
  for (let x of src_list) {
    if (comparison_list.includes(x)) {
      return true;
    }
  }
